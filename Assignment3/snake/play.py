...
def load_model(filename):
    ...
    try:
        # Pass custom_objects, similar to train.py, for robustness
        return keras.models.load_model(filename, custom_objects={'MSE': MeanSquaredError, 'mse': MeanSquaredError, 'RMSprop': RMSprop})
    except Exception as e:
        print(f"Error loading model '{filename}': {e}")
        print("Please ensure the model file is a compatible Keras H5 file (.h5).")
        print("If this error persists, the model file might be corrupted or incompatible with the current Keras version.")
        print("Consider re-saving the model from its original training environment or re-training.")
        raise
5
